...
import {
  Check, X, RefreshCw, Zap,
  Target, ArrowBigDown, PlusCircle
} from 'lucide-react';
...
function getDialColor(rate: number): string {
  if (rate >= 70) return '#c084fc';
  if (rate >= 50) return '#a855f7';
  if (rate >= 35) return '#e879f9';
  if (rate >= 20) return '#f472b6';
  return '#fb7185';
}
...
function getRarityMeta(rarity: string) {
  ...
  switch (r) {
    case 'mythic':    return { g: 'from-purple-600 via-fuchsia-500 to-indigo-600', tc: '#d8b4fe', icon: '✺', border: 'rgba(192,132,252,0.4)' };
    case 'legendary': return { g: 'from-violet-600 via-purple-500 to-fuchsia-500', tc: '#e9d5ff', icon: '❋', border: 'rgba(233,213,255,0.4)' };
    case 'epic':      return { g: 'from-indigo-600 via-purple-600 to-violet-600', tc: '#c7d2fe', icon: '◈', border: 'rgba(199,210,254,0.4)' };
    case 'rare':      return { g: 'from-purple-400 via-indigo-400 to-violet-400', tc: '#ddd6fe', icon: '✦', border: 'rgba(221,214,254,0.4)' };
    default:          return { g: 'from-slate-600 to-slate-700',                 tc: '#94a3b8', icon: '⬡', border: 'rgba(148,163,184,0.2)' };
  }
}
...
function SafeImage({ src, alt, className, rarityIcon }: { src?: string; alt: string; className?: string; rarityIcon: React.ReactNode }) {
  ...
  if (!src || error) return <div className="flex items-center justify-center w-full h-full opacity-20">{rarityIcon}</div>;

  return (
    <div className="relative w-full h-full flex items-center justify-center">
      {loading && <div className="absolute inset-0 flex items-center justify-center bg-white/5 rounded-xl"><RefreshCw className="w-5 h-5 animate-spin text-purple-400" /></div>}
      <img src={src} alt={alt} className={`${className} ${loading ? 'opacity-0' : 'opacity-100'} transition-opacity duration-300`} onLoad={() => setLoading(false)} onError={() => { setError(true); setLoading(false); }} />
    </div>
  );
}
...
function EliteCardPreview({ item, type = 'sacrifice', index = 0, total = 1 }: { item: any; type?: 'sacrifice' | 'target'; index?: number; total?: number }) {
  ...
  return (
    <motion.div initial={{ y: 50, opacity: 0, scale: 0.8 }} animate={{ y: 0, opacity: 1, scale: 1, rotate: rotation, x: xOffset }} exit={{ y: -100, opacity: 0, scale: 0.5 }} whileHover={{ y: -10, scale: 1.05, zIndex: 50 }} className="absolute" style={{ zIndex: index }}>
      <div className={`w-64 h-80 bg-[#131829] border-2 rounded-[2rem] p-4 relative overflow-hidden shadow-2xl transition-all duration-500 ${type === 'target' ? 'border-purple-400 shadow-[0_0_40px_rgba(168,85,247,0.3)]' : 'border-white/10'}`}>
        <div className={`absolute inset-0 opacity-20 bg-gradient-to-br ${meta.g}`} />
        <div className="relative z-10 h-full flex flex-col items-center">
          <div className="w-full flex justify-between items-start mb-4">
            <span className={`px-2 py-0.5 rounded text-[8px] font-black uppercase italic ${meta.g} text-white`}>{item.rarity}</span>
          ...
          <div className="flex-1 w-full flex items-center justify-center bg-black/40 rounded-2xl mb-4 relative overflow-hidden">
            <SafeImage src={item.image} alt={item.name} className="w-[85%] h-[85%] object-contain drop-shadow-[0_0_20px_rgba(255,255,255,0.2)]" rarityIcon={<div className="text-6xl">{meta.icon}</div>} />
          </div>
          <div className="text-center w-full">
            <div className="text-sm font-black text-white uppercase italic truncate mb-1">{item.name}</div>
            <div className="flex items-center justify-center gap-1.5">
              <span className="text-lg font-black text-fuchsia-300 italic">{item.value.toLocaleString()}</span>
...
function ProbabilityDial({ rate, dash, isUpgrading, rollValue }: { rate: number; dash: number; isUpgrading: boolean; rollValue: number | null }) {
  const R = 82, circ = 2 * Math.PI * R, col = getDialColor(rate);
  return (
    <div className="relative flex flex-col items-center select-none text-white" style={{ isolation: 'isolate' }}>
      <div style={{ width: 280, height: 280, position: 'relative' }}>
        <svg width="280" height="280" viewBox="0 0 200 200" style={{ overflow: 'visible' }}>
          <defs>
            <linearGradient id="arcG2" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stopColor={col} /><stop offset="100%" stopColor="#a855f7" /></linearGradient>
          </defs>
          <circle cx="100" cy="100" r={R} fill="none" stroke="#1e1b4b" strokeWidth="14" />
          <motion.circle 
            cx="100" cy="100" r={R} 
            fill="none" 
            stroke="url(#arcG2)" 
          ...
          />
          <circle cx="100" cy="100" r={56} fill="#0a0e1a" stroke="rgba(168,85,247,0.1)" strokeWidth="1" />
          <text x="100" y="108" textAnchor="middle" fill="white" fontSize="24" fontWeight="900" fontFamily="monospace">
          ...
          <motion.path 
            ...
            fill="white" 
            initial={{ rotate: 0 }} 
            animate={{ rotate: isUpgrading ? 360 * 3 + (rollValue || 0) : 0 }} 
            transition={isUpgrading ? { duration: 4.5, ease: [0.22, 1, 0.36, 1] } : { duration: 0.5, ease: "easeOut" }} 
            style={{ originX: 0.5, originY: 1, willChange: 'transform' }} 
            className="drop-shadow-[0_0_15px_rgba(255,255,255,0.8)]" 
          />
        </svg>
        <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-2 text-white"><ArrowBigDown className="w-8 h-8 fill-white animate-bounce" /></div>
      </div>
    </div>
  );
}
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-[110] flex items-center justify-center bg-black/90 backdrop-blur-xl p-6 text-white">
      <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} className="bg-[#131829] border-2 border-white/10 rounded-[3rem] p-10 max-w-sm w-full text-center relative overflow-hidden">
        <div className={`absolute top-0 left-0 w-full h-2 ${result.success ? 'bg-purple-500' : 'bg-red-500'}`} />
        <div className={`w-24 h-24 rounded-full flex items-center justify-center mx-auto mb-8 ${result.success ? 'bg-purple-500/20 text-purple-400 shadow-[0_0_30px_rgba(168,85,247,0.4)]' : 'bg-red-500/20 text-red-500 shadow-[0_0_30px_rgba(239,68,68,0.4)]'}`}>
        ...
        {result.success && result.item && (
          <div className="bg-black/40 border border-white/5 rounded-3xl p-6 mb-10">
            ...
            <div className="text-fuchsia-300 font-black italic">{result.item.value.toLocaleString()} PkCoins</div>
          </div>
        )}
        <Button variant="default" className="w-full py-6 rounded-2xl font-black italic uppercase bg-purple-600 hover:bg-purple-700" onClick={onClose}>
...
export function Upgrade() {
  ...
  const [frozenRate, setFrozenRate] = useState<number>(0);
  const [frozenDash, setFrozenDash] = useState<number>(0);
  ...
  const fetchCatalog = async () => {
    ...
    try {
      ...
      const formatted: TargetCard[] = (data || []).map(item => ({
        id: item.id.toString(), name: item.name, rarity: item.rarity,
        value: parseFloat(item.price), image: item.image_url
      }));
      setTargetPool(formatted);
    } catch (error) { console.error('Error loading catalog:', error); } finally { setLoading(false); }
  };
  ...
  const handleUpgrade = async () => {
    ...
    setFrozenRate(successRate);
    setFrozenDash(circ * (successRate / 100));

    const isSuccess = Math.random() * 100 < successRate;
    const successThreshold = successRate * 3.6;
    let targetAngle: number;
    if (isSuccess) {
      targetAngle = 5 + Math.random() * (successThreshold - 10);
    } else {
    ...
    setTimeout(async () => {
      ...
      } catch (error) { toast.error('Error al procesar upgrade.'); } finally { setIsUpgrading(false); }
    }, 4500);
  };

  return (
    <div className="min-h-screen py-24 relative overflow-hidden text-white">
      <div className="absolute inset-0 z-0 bg-cover bg-center bg-no-repeat" style={{ backgroundImage: 'url(/src/assets/fondo%20gengar.webp)' }} />
      <div className="absolute inset-0 z-0 bg-[#0a0e1a]/90" />
      <div className="container mx-auto px-6 max-w-[1600px] relative z-10">
        ...
        <div className="mb-20 relative h-[450px] w-full flex items-center justify-between px-12 lg:px-24">
          <div className="flex-1 flex items-center justify-center relative h-full">
            ...
            <AnimatePresence>
              {selectedCards.map((card, i) => <EliteCardPreview key={card.id} item={card} type="sacrifice" index={i} total={selectedCards.length} />)}
            </AnimatePresence>
          </div>
          <div className="z-50 shrink-0 transform scale-110 px-12">
            <div className="bg-[#131829]/80 border-2 border-white/10 rounded-full p-2 shadow-2xl relative">
              <ProbabilityDial 
                rate={isUpgrading ? frozenRate : successRate} 
                dash={isUpgrading ? frozenDash : successRate * (2 * Math.PI * 82 / 100)} 
          ...
          <div className="flex-1 flex items-center justify-center relative h-full">
            ...
            <AnimatePresence mode="wait">
              {selectedTarget ? <EliteCardPreview key={selectedTarget.id} item={selectedTarget} type="target" /> : (
